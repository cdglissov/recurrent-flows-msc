...
class Solver(object):
    ...
    def build(self):
        ...
        if self.multigpu and torch.cuda.device_count() > 1:
            ...
            self.model = nn.DataParallel(SRNN_RFN(self.params)).to(device)
        else:
            self.model = SRNN_RFN(self.args).to(device)
    ...
    def compute_loss(self, nll, kl_free_bit, kl, dims, t=1):
        ...
        self.kl_loss.append(float(self.beta*kl_store)/t)
    ...
    def train(self):
      ...
      for epoch_i in range(self.n_epochs):
          ...
          if self.epoch_i % 1 == 0:
            ...
            self.checkpoint('model.pt', self.epoch_i, epoch_loss) 
          ...
          if (self.earlystopping.best_loss < self.best_loss) and (self.epoch_i > 50):
            ...
            self.checkpoint('best_model.pt', self.epoch_i, epoch_loss) 
