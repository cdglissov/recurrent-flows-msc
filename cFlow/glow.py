...
from .glow_modules import ActNorm, Conv2dZeros, Conv2dNorm, InvConv, AffineCoupling, Squeeze2d, Split2d, BatchNormFlow, Conv2dResize
...
class GlowStep(nn.Module):
    def __init__(self, x_size, condition_size, args):
      ...
      self.affine =  AffineCoupling(x_size, condition_size, 
                                    ...
                                    one_condition = self.one_condition,
                                    )
...
class ListGlow(nn.Module):
    def __init__(self, x_size, condition_size, base_dist_size, args):
        ...
        for l in range(0, self.L):
            ...
            if l < (self.L-1):
                layers.append(Split2d(x_size, condition_size_cur, self.make_conditional, self.conditional_clamp_function, self.one_condition)) 
                Cx = Cx // 2 
                x_size = [Bx, Cx, Hx, Wx]

        self.glow_frame = nn.ModuleList(layers)

        if self.learn_prior == True and not self.one_condition:
        ...
        elif self.learn_prior == True and self.one_condition:
            (Cc, Hc, Wc) = condition_size[-1][1:]
            self.prior = nn.Sequential(
                Conv2dResize((Cc, Hc, Wc), out_size=(self.n_units_prior, Hx*2, Wx*2), norm=None),
                ActFun(self.non_lin_glow),
                Conv2dResize((self.n_units_prior, Hx*2, Wx*2), out_size=(self.n_units_prior//2, Hx, Wx), norm=None),
                ActFun(self.non_lin_glow),
                Conv2dZeros(in_channel=self.n_units_prior//2, out_channel=2*Cx)
                )
    ...
    def log_prob(self, x, condition, base_condition, logdet = None):
        ...
        prior = td.Normal(mean, nn.Softplus()(log_scale)+1e-6)
    ...
    def sample(self, z, condition, base_condition, num_samples = 32, temperature=0.8):
        ...
        with torch.no_grad():
          if z == None:
            ...
            prior = td.Normal(mean, (nn.Softplus()(log_scale)+1e-6)*temperature)
