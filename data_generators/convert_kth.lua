...
for _, class in pairs(classes) do
  ...
  for vid in paths.iterfiles(data_root .. '/raw/' .. class) do
    ...
    os.execute( ('~/tools/ffmpeg-git-20210110-amd64-static/ffmpeg -i %s/raw/%s/%s -r %d -f image2 -s %dx%d  %s/processed/%s/%s/image-%%03d_%dx%d.png'):format(data_root, class, vid, frame_rate, image_size, image_size, data_root, class, fname, image_size, image_size))
  end
end
