...
class MovingMNIST(object):
    ...
    def __init__(self, train, data_root, seq_len=20, num_digits=2, 
                 image_size=32,digit_size=28, deterministic=True, 
                 three_channels = True, step_length=4, normalize = False, 
                 make_target = False, set_starting_position = False, seed = None):
        path = data_root
...
class MovingMNIST_synchronized(object):
    """Synchronized motion for parameter analysis"""
    def __init__(self, train, data_root, seq_len=20, num_digits=2, image_size=32, digit_size=28, 
                 deterministic=True, three_channels = True, step_length=4, normalize = False, 
        ...
        self.data = datasets.MNIST(
                ...
                transform=transforms.Compose(
                    [transforms.Resize(self.digit_size, interpolation=1),
                     transforms.ToTensor()]))
        self.hit_boundary = np.zeros(self.seq_len)
        self.N = len(self.data) 
    ...
    def __getitem__(self, index):
        ...
        for n in range(self.num_digits):
            ...
            digit = digit.numpy()
            ds = digit.shape[1]
            sx =r.randint(image_size-ds)
            sy =r.randint(image_size-ds)
            r = np.random.RandomState(n*5)
            dx = 3
            dy = 3
            for t in range(self.seq_len):
                if sy < 0:
                    sy = 0 
                    self.hit_boundary[t]=n+1
                    if self.deterministic:
                        dy = -dy
                    else:
                        dy = r.randint(1, self.step_length+1)
                        dx = r.randint(-self.step_length, self.step_length+1)
                elif sy >= image_size-ds:
                    sy = image_size-ds-1
                    self.hit_boundary[t]=n+1
                    if self.deterministic:
                        ...
                        dy = r.randint(-self.step_length, 0)
                        dx = r.randint(-self.step_length, self.step_length+1)
                ...
                if sx < 0:
                    ...
                    if self.deterministic:
                        ...
                        dx = r.randint(1, self.step_length+1)
                        dy = r.randint(-self.step_length, self.step_length+1)
                elif sx >= image_size-ds:
                    self.hit_boundary[t]=n+1
                    sx = image_size-ds-1
                    if self.deterministic:
                        ...
                        dx = r.randint(-self.step_length, 0)
                        dy = r.randint(-self.step_length, self.step_length+1)
