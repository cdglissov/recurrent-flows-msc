...
def torch_to_FVD_format(torch_tensor):
  # Torch tensor in format [batch_size, video_length, channels, H, W] 
  # Return numpy_array in format [batch_size, video_length, H, W, channels] for FVD
  numpyarray = torch_tensor.permute(0, 1, 3, 4, 2).numpy()
  return numpyarray

def FVD(generated_list,true_list):
  """
  Format for input list of torch.tensors with dimension [batch, video_length, channels, Height, Width]
  So [tensor, tensor, ...., tensor]
  Batch size need to be divisible with 16. So 16, 32, 48, 64....
  """
  # Adapted and modified 
  # Source https://github.com/google-research/google-research/tree/master/frechet_video_distance
  # Licensed under the Apache License, Version 2.0 (the "License");
  # you may not use this file except in compliance with the License.
  # You may obtain a copy of the License at
  #
  ...
  with tf.Graph().as_default():
    ...
    for i in range(0, len(generated_list)):
      first_set_of_videos = tf.convert_to_tensor( torch_to_FVD_format( generated_list[i] ) )
      #
      second_set_of_videos = tf.convert_to_tensor( torch_to_FVD_format( true_list[i] ) )
    ...
    with tf.Session() as sess:
        ...
        for k in range(0,len(inputs)):
          results.append(sess.run(inputs[k]))
